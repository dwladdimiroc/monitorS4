 ...
 * semaphore. Task submissions require permits, task completions release
 * permits.
 * <p>
 * NOTE: you should either use the
 * {@link BlockingThreadPoolExecutorService#submit(java.util.concurrent.Callable)}
 * methods or the {@link BlockingThreadPoolExecutorService#execute(Runnable)}
 * method.
 * 
 */
public class BlockingThreadPoolExecutorService extends
	 ...
	 */
	public BlockingThreadPoolExecutorService(int parallelism,
			boolean fairParallelism, String threadName, int workQueueSize,
			final ClassLoader classLoader) {
		super();
		...
		ThreadFactory threadFactory = new ThreadFactoryBuilder()
				.setDaemon(true).setNameFormat(threadName)
		...
		workQueue = new LinkedBlockingQueue<Runnable>(workQueueSize
				+ parallelism);
		ThreadPoolExecutor eventProcessingExecutor = new ThreadPoolExecutor(
				parallelism, parallelism, 60, TimeUnit.SECONDS, workQueue,
				threadFactory, new RejectedExecutionHandler() {
					...
					public void rejectedExecution(Runnable r,
						...
						logger.error("Could not submit task to executor {}",
								executor.toString());
					}
				});
		eventProcessingExecutor.allowCoreThreadTimeOut(true);
		executorDelegatee = MoreExecutors
				.listeningDecorator(eventProcessingExecutor);
	}
	...
	public <T> ListenableFuture<T> submit(Callable<T> task) {
		...
		ListenableFuture<T> future = super
				.submit(new CallableWithPermitRelease<T>(task));
		return future;
	}
	...
	public <T> ListenableFuture<T> submit(Runnable task, T result) {
		...
		ListenableFuture<T> future = super.submit(
				new RunnableWithPermitRelease(task), result);
		return future;
	}
	...
	public ListenableFuture<?> submit(Runnable task) {
		...
		ListenableFuture<?> future = super
				.submit(new RunnableWithPermitRelease(task));
		return future;
	}
