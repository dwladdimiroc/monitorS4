...
/**
 * <p>
 * Base class for implementing processing in S4. All instances are organized as
 * follows:
 * <ul>
 ...
 * <li>PE instances do the actual work by processing any number of input events
 * of various types and emit output events of various types.
 * <li>To process events, {@code ProcessingElement} dynamically matches an event
 * type to a processing method. See
 * {@link org.apache.s4.core.gen.OverloadDispatcher} . There are two types of
 * processing methods:
 * <ul>
 * <li>{@code onEvent(SomeEvent event)} When implemented, input events of type
 * {@code SomeEvent} will be dispatched to this method.
 * <li>{@code onTrigger(AnotherEvent event)} When implemented, input events of
 ...
 * must be thread safe.
 * <li>The code in a PE instance is synchronized by the framework to avoid
 * concurrency problems.
 * <li>In some special cases, it may be desirable to allow concurrency in the PE
 * instance. For example, there may be several event processing methods that can
 * safely run concurrently. To enable concurrency, annotate the implementation
 * of {@code ProcessingElement} with {@link ThreadSafe}.
 * <li>PE instances never use the constructor. They must be initialized by
 * implementing the {@link #onCreate()} method.
 * <li>PE class fields are cloned from the prototype. References are also copied
 * which means that if the prototype creates a collection object, all instances
...
public abstract class ProcessingElement implements Cloneable {
	 ...
	 * the onEvent() and onTrigger() methods. To execute concurrently with other
	 * methods, the {@link ProcessingElement} subclass must be annotated with
	 * {@link ThreadSafe}.
	 * 
	 ...
	 * 
	 * <p>
	 * <b>Fields initialized in the class constructor are shared by all PE
	 * instances.</b>
	 * </p>
	 */
	...
	/**
	 * Makes this PE a singleton. A single PE instance is eagerly created and
	...
	/**
	 * Checks the trigger for this event type. Creates an inactive trigger if no
	 * trigger is found after recursively exploring the event class hierarchy.
	 * An inactive trigger never triggers.
	 * 
	 * @return true if trigger is reached, false if trigger is not ready yet or
	...
	protected void initPEPrototypeInternal() {
		...
		if (getClass().isAnnotationPresent(ThreadSafe.class) == true) {
			...
			// TODO: this doesn't seem to be working. isannotationpresent always
			// returns false.
	...
	/**
	 * Helper method to be used by PE implementation classes. Sends an event to
	 * all the target streams.
	 * 
	 */
	...
	/**
	 * By default, the state of a PE instance is considered dirty whenever it
	 * processed an event. Some event may actually leave the state of the PE
	 * unchanged. PE implementations can therefore override this method to
	 * accommodate specific behaviors, by managing a custom "dirty" flag.
	 * 
	 * <b>If this method is overriden, {@link #clearDirty()} method must also be
	 * overriden in order to correctly reflect the "dirty" state of the PE.</b>
	 */
	...
	/**
	 * Dirty state is cleared after the PE has been serialized. PE
	 * implementations that maintain their "dirty" flag must override this
	 * method by clearing their internally managed "dirty" flag.
	 * 
	 * <b>If this method is overriden, {@link #isDirty()} must also be overriden
	 * in order to correctly reflect the "dirty" state of the PE.</b>
	 */
