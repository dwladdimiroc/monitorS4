...
/**
 * Utility class for centralizing system runtime metrics, such as information
 * about event processing rates, cache eviction etc...
 */
@Singleton
public class S4Metrics {
	...
	private final Meter eventMeter = Metrics.newMeter(ReceiverImpl.class,
			"received-events", "event-count", TimeUnit.SECONDS);
	private final Meter bytesMeter = Metrics.newMeter(ReceiverImpl.class,
			"received-bytes", "bytes-count", TimeUnit.SECONDS);

	private final Meter localEventsMeter = Metrics.newMeter(Stream.class,
			"sent-local", "sent-local", TimeUnit.SECONDS);
	private final Meter remoteEventsMeter = Metrics.newMeter(Stream.class,
			"sent-remote", "sent-remote", TimeUnit.SECONDS);
	...
	private final Meter droppedInSenderMeter = Metrics.newMeter(
			SenderImpl.class, "dropped@sender", "dropped@sender",
			TimeUnit.SECONDS);
	private final Meter droppedInRemoteSenderMeter = Metrics.newMeter(
			SenderImpl.class, "dropped@remote-sender", "dropped@remote-sender",
			TimeUnit.SECONDS);
	...
	private void init() {
		...
		if (Strings.isNullOrEmpty(metricsConfig)) {
			...
			if (!matcher.matches()) {
				...
				if (group1.startsWith("csv")) {
					...
					logger.info(
							"Reporting metrics through csv files in directory [{}] with frequency of [{}] [{}]",
					...
					TimeUnit timeUnit = TimeUnit.valueOf(matcher.group(3));
					logger.info(
							"Reporting metrics on the console with frequency of [{}] [{}]",
		...
		Metrics.newGauge(Stream.class, "local-vs-remote", new Gauge<Double>() {
			...
			public Double value() {
				...
				return localEventsMeter.oneMinuteRate()
						/ remoteEventsMeter.oneMinuteRate();
			}
		});
	...
	public void createCacheGauges(ProcessingElement prototype,
		...
		Metrics.newGauge(prototype.getClass(), prototype.getClass().getName()
				+ "-cache-entries", new Gauge<Long>() {
		...
		Metrics.newGauge(prototype.getClass(), prototype.getClass().getName()
				+ "-cache-evictions", new Gauge<Long>() {
	...
	public void createStreamMeters(String name) {
		...
		dequeuingStreamMeters.put(name, Metrics.newMeter(Stream.class,
				"dequeued@" + name, "dequeued", TimeUnit.SECONDS));
		droppedStreamMeters.put(name, Metrics.newMeter(Stream.class, "dropped@"
				+ name, "dropped", TimeUnit.SECONDS));
		streamQueueFullMeters.put(name, Metrics.newMeter(Stream.class,
				"stream-full@" + name, "stream-full", TimeUnit.SECONDS));
	}
	...
	public void createRemoteStreamMeters(String remoteClusterName,
			int partitionCount) {
		Meter[] meters = new Meter[partitionCount];
		for (int i = 0; i < partitionCount; i++) {
			meters[i] = Metrics.newMeter(RemoteSender.class, "remote-sender@"
					+ remoteClusterName + "@partition-" + i, "sent",
					TimeUnit.SECONDS);
		}
	...
	public static class CheckpointingMetrics {
		...
		static Meter rejectedSerializationTask = Metrics.newMeter(
				CheckpointingMetrics.class,
				"checkpointing-rejected-serialization-task",
				"checkpointing-rejected-serialization-task", TimeUnit.SECONDS);
		static Meter rejectedStorageTask = Metrics.newMeter(
				CheckpointingMetrics.class,
				"checkpointing-rejected-storage-task",
				"checkpointing-rejected-storage-task", TimeUnit.SECONDS);
		static Meter fetchedCheckpoint = Metrics.newMeter(
				CheckpointingMetrics.class, "checkpointing-fetched-checkpoint",
				"checkpointing-fetched-checkpoint", TimeUnit.SECONDS);
		static Meter fetchedCheckpointFailure = Metrics.newMeter(
				CheckpointingMetrics.class,
				"checkpointing-fetched-checkpoint-failed",
				"checkpointing-fetched-checkpoint-failed", TimeUnit.SECONDS);
