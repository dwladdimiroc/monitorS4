...
public class Distribution {
	...
	public List<Integer> dynamicTweets(String path) {
		Words read = new Words();
		Map<Integer, Integer> cantTweets = read.readCantTweets(path);
		List<Integer> cantTweetsNorm = new ArrayList<Integer>();

		for (int i = 0; i < 4300; i++) {
			if (cantTweets.containsKey(i)) {
				int auxCont = cantTweets.get(i);
				...
				cantTweetsNorm.addAll(Arrays.asList(arrayNumber));
			} else {
				Integer[] arrayNumber = new Integer[100];
				for (int j = 0; j < arrayNumber.length; j++)
					arrayNumber[j] = 0;
